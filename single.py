...
def get_user_stats(group_id: int, user_id: int) -> sqlite3.Row | None:
...
def get_leaderboard(group_id: int, limit: int = TOP_N) -> list[sqlite3.Row]:
    ...
    with db_cursor() as cur:
        cur.execute("""
            SELECT
                ...
                s.message_count,
                s.total_points,
                RANK() OVER (PARTITION BY s.group_id ORDER BY s.total_points DESC, s.message_count DESC, s.user_id ASC) AS rank
            FROM stats s
            JOIN users u ON u.user_id = s.user_id
            WHERE s.group_id = ?
            ORDER BY s.total_points DESC, s.message_count DESC, s.user_id ASC
            LIMIT ?
        """, (group_id, limit))
        return cur.fetchall()


def get_user_rank(group_id: int, user_id: int) -> dict | None:
    """
        ...
        cur.execute("""
            SELECT
                ...
                (
                    ...
                    WHERE s2.group_id = ?
                    AND (
                        s2.total_points > s.total_points
                        OR (
                            s2.total_points = s.total_points
                            AND s2.message_count > s.message_count
                        )
                        OR (
                            s2.total_points = s.total_points
                            AND s2.message_count = s.message_count
...
# ─────────────────────────────────────────────
def message_is_valid(update: Update) -> tuple[bool, str]:
    """
...
"] or "Unknown User"
        name: str = escape_md(raw_name[:20])
        pts: int = int(row["total_points"])
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Log unexpected telegram errors cleanly."""
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if chat is None or msg is None or chat.type != ChatType.PRIVATE:
    ...
    if not bot_username:
        await msg.reply_text("⚠️ Unable to determine bot username right now.")
        return

    # Correct plain URL for Telegram add-to-group deep link
    ...
    welcome_text = (
        ...
        f"• Each valid message earns *{POINTS_PER_MESSAGE} points*\n"
    ...
    logger.info("Sent /start welcome to user %s", update.effective_user.id if update.effective_user else "unknown")


async def cmd_leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Handle /leaderboard in a group chat.
    Displays top-N users and a 'Find My Place' button.
    """
    chat = update.effective_chat
    msg = update.effective_message
    ...
    if chat.type not in (ChatType.GROUP, ChatType.SUPERGROUP):
        await msg.reply_text("⚠️ This command only works inside a group.")
        return
...
async def cb_find_my_place(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        _, raw_group_id = (query.data or "").split(":")
    ...
    if result is None:
        ...
        pts = int(result["total_points"])
        msgs = int(result["message_count"])
        ...
        msg = (
            ...
            f"💬 Messages: {msgs}\n"
            "─────────────────\n"
            "Keep it up! Every message counts."
        )
...
async def handle_group_message(
    ...
    if not valid:
        logger.debug(
            "Message rejected [%s] from user %s in group %s",
    ...
    if not passes:
        logger.debug(
            "Message skipped [%s] for user %s in group %s",
            reason, user_id, group_id
        )
        return
...
async def handle_bot_added(
    ...
    bot_was_added = any(member.id == me.id for member in new_members)
    ...
    welcome = (
        ...
        "📌 *How to use me:*\n"
        f"• Just chat naturally — every valid message earns *{POINTS_PER_MESSAGE} points*\n"
...
def build_application() -> Application:
    ...
    # Runs on all group messages that are NOT commands, NOT status updates
    app.add_handler(
        MessageHandler(
